...
export const fetchProduct = async (searchParams: ProductSearchParmasProps) => {
    const { pageNum, productFilter } = convertObjToProductFilter(searchParams);
    const data: ProductFetchResponseProps = await fetch(`${process.env.PRODUCT_API_URL}/api/product/category?page=${pageNum}`, {
        method: 'POST',
        headers: {
